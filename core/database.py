        ...
        try:
            conn.execute(
                ...
                (
                    ...
                    int(bool(data.get("auto_start", False))),
                    now,
                    now,
                ),
            )
    ...
    with _db_lock:
        ...
        try:
            conn.execute(
                """
                UPDATE channels SET
                    name=?, description=?, source=?, logo_path=?, logo_position=?,
                    logo_scale=?, enabled=?, auto_start=?, updated_at=?
                WHERE id=?
                """,
                (
                    merged["name"],
        ...
        try:
            conn.execute(
                ...
                (
                    ...
                    d["extra_args"], int(bool(d["auto_start"])), now_iso(), output_id,
                ),
            )
            conn.commit()
        finally:
