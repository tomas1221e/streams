...
export default function FilterSelect(props) {
	...
	if (!hwaccel) {
		for (let c of filterRegistry.List()) {
			// Built-in FFmpeg filters are checked against Core skills. Onside
			// also provides UI-level filters that are composed from standard
			// FFmpeg filters and therefore don't have their own Core skill name.
			const onsideFilter = ['color', 'logo'].includes(c.filter);
			if (props.availableFilters.includes(c.filter) || onsideFilter) {
...
FilterSelect.defaultProps = {
	...
	availableFilters: [],
	coreAddress: '',
	onStore: async function (name, data) {
		return '';
	},
