...
const createInputsOutputs = (sources, profiles, requireVideo = true) => {
	...
	for (let profile of profiles) {
		...
		let complexFilter = '';
		let videoMap = index + ':' + stream.stream;

		if (profile.video.encoder.coder !== 'copy') {
			let filter = profile.video.filter.graph || '';
			const hasLogo = filter.indexOf('__onside_logo__') !== -1;

			if (hasLogo) {
				const settings = profile.video.filter.settings?.logo?.settings || {};
				const logoUrl = settings.logoUrl || '';
				const position = settings.position || 'top-right';
				const size = Math.max(0.05, Math.min(0.5, Number(settings.size || 0.18)));
				const opacity = Math.max(0.1, Math.min(1, Number(settings.opacity || 0.9)));

				filter = filter
					.split(',')
					.filter((entry) => entry !== '__onside_logo__')
				...
				const escapedLogoUrl = String(logoUrl)
					.replace(/\\/g, '\\\\')
					.replace(/:/g, '\\:')
					.replace(/'/g, "\\'");
				const labelId = outputs.length;
				const logoSource = `movie='${escapedLogoUrl}':loop=0,setpts=N/FRAME_RATE/TB,format=rgba,colorchannelmixer=aa=${opacity},scale=trunc(iw*${size}/2)*2:-1[onside_logo_${labelId}]`;
